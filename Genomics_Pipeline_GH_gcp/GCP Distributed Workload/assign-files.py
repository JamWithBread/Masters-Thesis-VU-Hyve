...
def assign_files(vm_index, group_info, all_files, limit, vm_group_offset):
    num_vms = len(group_info)+vm_group_offset
    ...
    assigned_files = all_files[start_index:stop_index]


    if limit !=0:
    	if limit < len(assigned_files):
    		assigned_files = assigned_files[:limit]
...
def main():
	...
	#### SET LIMIT OF FILES HERE (set to 0 if no limit desired) ####
	limit = 0
	### If you want to leave some unprovisioned files available to be assigned to a separate VM or group -> makes the index ranges a fraction smaller
	vm_group_offset = 1
	...
	assigned_files = assign_files(vm_index, group_info, assignable_files, limit, vm_group_offset)
